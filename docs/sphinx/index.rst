...
:mod:`PollingSensors <snsary.sources.polling_sensor>` are a type of Source that can ``start`` and ``stop`` i.e. a :mod:`Service <snsary.utils.service>`. Every instance of a :mod:`Service <snsary.utils.service>` is recorded in ``Service.instances``, so the following should be equivalent: ::
...
:mod:`Outputs <snsary.outputs.output>` can also be :mod:`Services <snsary.utils.service>`. These concepts are all you need to build an app, but Snsary has a few more tools to hide some of the complexity and reduce the amount of code needed:
