"""
Wrapper for most Adafruit sensor objects, which usually have one or more properties to retrieve the current values of the sensor. AdafruitSensor polls a given sensor object and publishes the latest value of each property as a :mod:`Reading <snsary.models.reading>`.

Most Adafruit sensors come with a driver written for CircuitPython, which can be used in pure Python via `the Adafruit Blinka library <https://github.com/adafruit/Adafruit_Blinka>`_. You will need to separately install the CircuitPython library for each sensor you want to scrape. For each sensor, you will then need to manually create and configure an instance of AdafruitSensor.
